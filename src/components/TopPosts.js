...
const TopPosts = () => {
  ...
  useEffect(() => {
    setTranslateClass(`-translate-x-${1 * cardIndex}/3`);
  }, [cardIndex]);
  ...
  return (
    <section>
      ...
      <div className="w-full relative overflow-hidden">
        <button
          className="bg-black text-white px-2 py-5 rounded-r-lg opacity-50 hover:opacity-90 absolute top-1/2 z-10 left-0 -translate-y-1/2 lg:hidden"
          onClick={handlePrevCard}
        >
          <BsChevronLeft size={20} />
        </button>
        <button
          className="bg-black text-white px-2 py-5 rounded-l-lg opacity-50 hover:opacity-90 absolute top-1/2 z-10 right-0 -translate-y-1/2 lg:hidden"
          onClick={handleNextCard}
        >
          <BsChevronRight size={20} />
        </button>
        <div
          className={`grid grid-cols-2 grid-rows-2 gap-3 max-lg:grid-cols-3 max-lg:gap-0 max-lg:grid-rows-1 max-lg:w-[300%] max-lg: max-lg:${translateClass} transition`}
        >
